...
"use strict";
ll.registerPlugin("Broadcast", "广播", [1, 0, 0]);

const config = new JsonConfigFile("plugins/Broadcast/config.json");
const interval = config.init("interval", 600);
const message = config.init("message", []);
config.close();
let index = 0;
setInterval(() => {
    mc.broadcast(message[index]);
